<script lang="ts">
	...
	const submitPrompt = async (userPrompt, { _raw = false } = {}) => {
		...
		if (selectedModels.includes('') && (($settings?.workspaceRoutingMode ?? 'auto') !== 'auto' || atSelectedModel)) {
			toast.error($i18n.t('Model not selected'));
			return;
		}
		...
		const messages = createMessagesList(history, history.currentId);
		const _files = structuredClone(files);
		const routePlan = await resolveWorkspaceRoute(userPrompt, _files);
		const resolvedModelId =
			routePlan?.selected_model_id ??
			atSelectedModel?.id ??
			selectedModels.find((modelId) => modelId && $models.some((model) => model.id === modelId));
		...
		let userMessage = {
			...
			models: routePlan?.selected_model_id ? [routePlan.selected_model_id] : selectedModels,
			...(routePlan ? { info: { workspace_route: routePlan } } : {})
		};
		...
		await sendMessage(history, userMessageId, { newChat: true, routePlan });
	};
	...
	const sendMessage = async (
		...
		const routeSelectedModel =
			routePlan?.selected_model_id !== undefined
				? $models.find((model) => model.id === routePlan.selected_model_id)
				: undefined;
		...
		let selectedModelIds = modelId
			? [modelId]
			: routeSelectedModel !== undefined
				? [routeSelectedModel.id]
	...
	const getFeatures = (routePlan: WorkspaceRoute | null = null) => {
		let features = {};
		...
		if (routePlan?.features) {
			features = { ...features, ...routePlan.features };
		}
	...
	const sendMessageSocket = async (
		...
		responseMessageId,
		_chatId,
		routePlan: WorkspaceRoute | null = null
	) => {
		...
		const res = await generateOpenAIChatCompletion(
			localStorage.token,
			{
				...
				features: getFeatures(routePlan),
				variables: {
					...getPromptVariables(
						$user?.name,
				...
				background_tasks: {
					...
					(selectedModels[0] === model.id ||
						atSelectedModel !== undefined ||
						routePlan?.selected_model_id === model.id)
						? {
